...
function getQuizzes(){
    ...
    try {
        $statement = $dbh->prepare(
            'select Users.username as authorUsername,Quizzes.* '. 
            'from Quizzes join Users on Users.id = authorId');
        $statement->execute();
 ...
 * looks like {"success": false, "error": <error message>}.
 */
function updateQuiz($quizId, $name){
    global $dbh;
    try {
        // $dbh->exec('update Quizzes set name = "'. $name .'" where id = '. $quizId);
        // $dbh->exec('update Quizzes set name = "Foobar\'s quiz1 as been pawned" where id = 1 -- " where id = '. $quizId);
        $statement = $dbh->prepare(
            'update Quizzes set name = :name where id = :quizId');
        $statement->execute([
            ':quizId' => $quizId,
            ':name' => $name
        ]);
        ...
        return error("There was an error updating the quiz: $e");
    }
}
...
function getQuizItemsForQuiz($quizId){
    ...
    try {
        $statement = $dbh->prepare(
            'select * from QuizItems where quizId = :quizId');
